...
contract TheDexerFlash is FlashLoanReceiverBase {
    ...
    function executeOperation(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            IERC20(assets[i]).safeApprove(address(LENDING_POOL), amountToReturn);
            IERC20(assets[i]).safeTransfer(address(LENDING_POOL), amountToReturn);
        }
